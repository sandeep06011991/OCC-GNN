...
// 24 Hours to come up with more memory efficient version
// Only after some testing is done.
// Take a break to disconnect
...
void partition_edges_push(int*  partition_map,\ // partition_map assigning each vertex ID to one GPU
    long * out_nodes, size_t out_nodes_size,\ // Sample layer out nodes indexed into the graph
      ...
      // Partitioned graphs such that indptr_map[dest, src]
       	  	bool last_layer, void ** storage_map, int NUM_GPUS, int gpu_id){
    ...
    while(start < end){
      ...
      for(int nb_idx = 0; nb_idx < nbs; nb_idx ++ ){
        ...
        if((p_nd1 == p_nd2)){
          ...
          ((long *)&index_edge_local[num_edges])[offset_edge_start + nb_idx] = 1;
          // Mark only if not in the outnode list
          // Makes setting self nodes easier
          if(!(nd2_idx < out_nodes_size)){
            ((long *)&index_in_nodes[in_nodes_size])[nd2_idx] = 1;
            }
           p_nbs[p_nd1] ++;
           continue;
        }
          ...
          // Not the same partition but part of our redundant store.
      		if(((int *)storage_map[p_nd1])[nd2]!= -1) && (p_nd1 == gpu_id){
        ...
        ((long *)&index_edge_remote[remote_offset  * num_edges])[offset_edge_start + nb_idx] = 1;
