...
def producer(queue,size,proc_id,no_objects,reuse_queue):
    creation_time = 0
    producer_time = 0
...
if __name__ == "__main__":
    ...
    for i in range(no_procs):
        producer_process = mp.Process(target=(producer), \
                      args=(queue, size, i, no_objects, reuse_queue))
        producer_process.start()
        pp.append(producer_process)
    consumer_process = mp.Process(target = (consumer),args = (queue,no_objects, no_procs, reuse_queue))
