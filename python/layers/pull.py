...
class Pull(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, local_t, device_id, layer_id ,pull_from_nds_size,\
                to_tensor_offset):
        recv = []
        recv_g = []
        send_dict = []
...
def pull(bipartite_graph, local_out):
    # of Higher size.
    new_out = Pull.apply(local_out, bipartite_graph.push_to_ids, bipartite_graph.pull_from_offsets)
    returns new_local_out
