...
class PaCache:
    ...
    def __init__(self,graph,nfeat,max_size,dev_id):
            ...
            self.dev_id = dev_id
            print("NOTE: Assuming bidirectional graphs using in degree.")
    ...
    def load_subtensor(self,nfeat, labels, seeds, input_nodes, dev_id):
        ...
        if(cache_hit.shape[0] == 0):
            # print("no cache hit")
            with profiler.record_function("cache_refresh"):    
            # if  True:
...
def run(proc_id, n_gpus, args, devices, data):
    ...
    # Split train_nid
    print(train_nid.shape,"Train noid", len(train_nid), n_gpus)
    ...
    my_schedule = schedule(warmup = 1)
    with torch.autograd.profiler.profile(enabled=(0==dev_id), use_cuda=True) as prof:
    #if True:
        for epoch in range(args.num_epochs):
            ...
            while(True):
                ...
                #torch.cuda.nvtx.range_push("sample")
                    #print("Attemtong to pop sampe")
                    step, (input_nodes, seeds, blocks) = next(it)
                    nodes_done += seeds.shape[0]
                    print("nodes progress", nodes_done, train_nid.shape[0])
    ...
    if proc_id == 0:
        ...
        print("Avg forward time: {}sec, device {}".format(sum(forward_time[1:])/(num_epochs - 1), dev_id))
        print(forward_time)
...
if __name__ == '__main__':
    ...
    g,p_map,num_classes =  get_process_graph(args.graph)
