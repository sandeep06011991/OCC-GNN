...
def create_metis_file(graphname, num_partitions):
    ...
    print(dg_graph)
    pmap = torch.zeros(num_nodes)
    partitioned_graphs = dgl.metis_partition(dg_graph, num_partitions)
    for p_id in range(num_partitions):
        pmap[partitioned_graphs[p_id].ndata['orig_id']] = p_id
    with open(TARGET_DIR + '/partition_map_opt_{}.bin'.format(num_partitions),'wb') as fp:
        fp.write(pmap.astype(np.intc).tobytes())
    print("Write done !!!")    
