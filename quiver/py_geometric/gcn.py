...
def run(rank, args,  data):
    ...
    edge_index = torch.stack(graph.edges('uv'))
    # Create PyTorch DataLoader for constructing blocks
    csr_topo = quiver.CSRTopo(edge_index)

    quiver_sampler = GraphSageSampler(csr_topo, sizes=[20,20,20], device=rank, mode='GPU')
    ...
    for epoch in range(args.num_epochs):
        ...
        dataloader_i = iter(train_loader)
        edges_computed = 0
        data_movement = 0
        ...
        try:
            while True:
                ...
                for adj in adjs:
                    print(type(adj))
                    #edges_computed += adj.shape[0] 
                batch_time[SAMPLE_START_TIME] = t1
                batch_time[GRAPH_LOAD_START_TIME] = time.time()

                optimizer.zero_grad()
                batch_time[DATALOAD_START_TIME] = time.time()
                e0.record()
                torch.cuda.nvtx.range_push("fetch")
                batch_feat = nfeat[n_id]
                torch.cuda.nvtx.range_pop()
                e1.record()
                batch_time[DATALOAD_END_TIME] = time.time()
                e3.record()
                out = model(batch_feat, adjs)
                e4.record()
                #print(n_id, labels, batch_size, out)
                loss = F.nll_loss(out, labels[n_id[:batch_size]])
                loss.backward()
                e5.record()
        ...
        approx_acc = total_correct / (len(train_loader) * args.batch_size)
