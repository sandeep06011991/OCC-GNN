...
def run_experiment_occ(model, skip_shuffle = False, load_balance = False, use_uva = False):
    ...
    for graphname, hidden_size, fsize, batch_size in settings:
        for hidden_size in hidden_sizes:
            out = run_occ(graphname, model,  cache, hidden_size, fsize,\
                    batch_size, num_layers, num_partition, fanout, skip_shuffle, load_balance, use_uva)
            with open(OUT_DIR + '/basic/occ_upgrade{}.txt'.format(SYSTEM),'a') as fp:
                fp.write("{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}\n".\
                format(graphname , SYSTEM, cache, hidden_size, fsize, batch_size,\
                    num_partition, num_layers, model, fanout.replace(",","-"), MODE, out["sample_get"], \
                    out["movement_graph"], out["movement_feat"], out["forward"], out["backward"], \
                     out["epoch"], out["accuracy"], out["data_moved"], out["edge_moved_avg"], out["edge_moved_skew"],\
                             skip_shuffle, load_balance, use_uva))
...
if __name__ == "__main__":
    with open(OUT_DIR + '/basic/occ_upgrade{}.txt'.format(SYSTEM),'a') as fp:
        ...
        fp.write("graph,system,cache,hidden-size,fsize,batch-size,"+\
            ...
            "model,fanout,UVA, sample_get,move-graph,move-feature,forward,backward,"+\
                "epoch_time,accuracy,data_moved,edges_comp(avg), edge_comp(skew),shuffle_skip,load_balance\n")
    run_experiment_occ("gat", skip_shuffle = False, load_balance = True, use_uva= True)
    #run_experiment_occ("gat", skip_shuffle = False, load_balance = True, use_uva= False)
    run_experiment_occ("gat", skip_shuffle = False, load_balance = False, use_uva= True)
    #run_experiment_occ("gat", skip_shuffle = False, load_balance = False, use_uva= False)
