...
def write_table(cache):
    quiver = pandas.read_csv('quiver_upgrade.txt', sep = ',', header = 0, index_col = False)
    #groot_cpu = pandas.read_csv('occ_cpuP4.txt', sep = '|', header = 0, index_col =  False)
    groot_gpu = pandas.read_csv('occ_upgradeP4.txt', sep = ',', header = 0, index_col = False)
    ...
    with open('relative.txt', 'w') as fp:
        fp.write("graph,hidden,model,batch,cache,Quiver-GPU(Sample)" +\
                ",Quiver-GPU(Load),Quiver-GPU(Train),Quiver-GPU(Epoch)," +\
                "GRoot-GPU(Sample),GRoot-GPU(Train),Quiver-GPU(Epoch)\n")
    for graph in graphs:
        for batch in batches:
            for hidden in [False]:
                for model in ["gcn", "gat"]:
                    ...
                    #print(quiver['hidden-size'] == hidden, quiver['hidden-size'], hidden) 
                    ...
                    if(len(quiver_gpus) == 1):
                        ...
                        quiver_gpu_train = float(quiver_gpus["forward"].iloc[0]) + float(quiver_gpus["backward"].iloc[0])
                    ...
                    if(len(_groot_gpu) == 1):
                        ...
                        groot_gpu_train = float(_groot_gpu["forward"].iloc[0]) + float(_groot_gpu["backward"].iloc[0])
                    ...
                    with open('relative.txt', 'a') as fp:
                        fp.write(("{},{},{},{},{},{},{}," + \
                            ...
                            graph, hidden, model, batch, cache, quiver_gpu_sample, quiver_gpu_load,\
                                quiver_gpu_train, quiver_gpu_epoch,\
                                groot_gpu_sample, groot_gpu_train, groot_gpu_epoch))
