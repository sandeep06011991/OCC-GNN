from baselines.occ_cpu import run_occ_cpu
import subprocess
import re
import sys
import os
...
def run_experiment_occ(model, skip_shuffle = False):
    ...
    with open(OUT_DIR + '/hidden/occ_cpu{}.txt'.format(SYSTEM),'a') as fp:
        fp.write("sha:{}, dirty:{}\n".format(sha,dirty))
        fp.write("graph | system | cache |  hidden-size | fsize  | batch-size |"+\
            ...
            " epoch_time | accuracy | data_moved | edges_computed| shuffle_skip | CPU_SLICE\n")
    for graphname, hidden_size, fsize, batch_size in settings:
        for hidden_size in hidden_sizes:
            out = run_occ_cpu(graphname, model,  cache, hidden_size, fsize,\
            ...
            with open(OUT_DIR + '/hidden/occ_cpu{}.txt'.format(SYSTEM),'a') as fp:
                ...
                format(graphname , SYSTEM, cache, hidden_size, fsize, batch_size,\
                    ...
                    out["movement_graph"], out["movement_feat"], out["forward"], out["backward"], \
                     out["epoch"], out["accuracy"], out["data_moved"], out["edges_moved"], skip_shuffle, True))
...
if __name__ == "__main__":
    ...
    run_experiment_occ("gat-pull", skip_shuffle = False)
