...
def run_experiment_quiver( model, cache , sample_gpu):
    ...
    with open('{}/depth/quiver.txt'.format(OUT_DIR),'a') as fp:
        ...
        fp.write("graph | system | cache |  hidden-size | fsize  |" + \
            " batch-size | model  | layers | fanout |  GPU_Sample | sample_get | move-data | forward |" +\
    ...
    for graphname, fsize, batch_size in settings:
        for num_layers in layer_sizess:
            ...
            out = run_quiver(graphname, model ,no_epochs, cache, hidden_size, fsize, batch_size, num_layers, fanout,sample_gpu)

            with open('{}/depth/quiver.txt'.format(OUT_DIR),'a') as fp:
                fp.write(("{} | {} | {} | {} | {} | {} | {} "+\
                       "| {} | {} | {} | {} | {} | {} | {} | {} |"+\
                       " {} | {}  | {} |  {} \n").format(graphname , "quiver", cache, hidden_size, fsize,\
                        4 * batch_size, model, num_layers, fanout, sample_gpu, out["sample_get"], out["movement_data"], \
