...
void aggregate_gat(std::vector<long>& layer_out_nds, std::vector<long> & layer_in_nds, \
...
int naive_flow_up_sample_gat(Sample &s, int number_of_nodes){
   std::vector<int> in_f;
   ...
   for(int i=s.num_layers - 1; i >=0; i--){
     aggregate_gat(s.block[i]->layer_nds, s.block[i+1]->layer_nds, \
...
void pull_own_node_gat(BiPartite *bp,
...
int sample_flow_up_ps_gat(PartitionedSample &s,
  ...
  for(int i =  s.num_layers-1  ; i>=0; i--){
    ...
    for(int j = 0; j < num_gpus; j++){
      pull_own_node_gat(layer.bipartite[j], out[j], in[j]);
    }
    // Swap
    for(int j=0;j<num_gpus; j++){
...
void test_sample_partition_consistency_gat(Sample &s, PartitionedSample &ps,
  std::vector<int> local_storage[8], int gpu_capacity[8], int num_nodes,\
      int num_gpus){
    int correct = naive_flow_up_sample_gat(s, num_nodes);
    for(int i=0;i<num_gpus; i++){
      assert(local_storage[i].size() == gpu_capacity[i]);
    }
    std::cout <<"reached here !\n";
    int out = sample_flow_up_ps_gat(ps, local_storage, num_gpus);
