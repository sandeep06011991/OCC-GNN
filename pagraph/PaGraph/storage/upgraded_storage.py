...
class GraphCacheServer:
  ...
  def get_feat_from_server(self, nids, embed_names, to_gpu=False):
    ...
    if to_gpu:
      frame = {name: self.graph.ndata[name][nids].cuda(self.gpuid, non_blocking=True)\
                   for name in embed_names}
    else:
      frame = {name: self.graph.ndata[name][nids] for name in embed_names}
    return frame
  ...
  def fetch_data(self, nodeflow):
    ...
    nf_nids = nf_nids.cuda(self.gpuid)
    ...
    for i in range(nodeflow.num_layers):
      ...
      with nvtx.annotate('cache-assign'):
